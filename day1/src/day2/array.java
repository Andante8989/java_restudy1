...
public class array {
	public static void main(String[] args) {
		// 1부터 10까지의 숫자들 중 홀수들의 모임인 1, 3, 5, 7, 9 라는 집합을 생각해 보자
		// 이것들을 숫자나 문자열만으로 표현 하기는 쉽지가 않다
		// 이러한 문제를 해결하기 위해 배열이 있다
		// 1부터 10까지의 숫자들 중 홀수들의 집합은 다음과 같이 int 배열로 표현 할 수 있다
		int[] odds = {1, 3, 5, 7, 9};
		
		// 배열은 자료형 타입 바로 옆에 [] 기호를 사용하여 표현한다. 위 예제와 같이 int자료형의 배열은 int[]로 표현
		...
		weeks2[5] = "토";
		weeks2[6] = "일";
		// 배열의 길이를 먼저 설정하여 배열 변수를 먼저 생성한 다음 그 값은 나중에 대입하는 방법이다. 단, 위 예처럼 초기값 없이
		// 배열 변수를 만들때에는 반드시 길이에 대한 숫자값이 필요하다 즉, 다음과 같은 코드는 불가능하다
		// String[] weeks = new String[]; 길이에 대한 숫자값이 없으므로 컴파일 오류 발생
		
		// 배열의 값은 어떻게 접근하는가?
		// 위에서 만든 요일의 배열줄 "목"에 해당되는 값을 얻으려면 다음과 같이 인덱싱을 이용해야 한다
		System.out.println(weeks[3]);
		// weeks[3]은 weeks 배열의 네 번째 항목을 의미한다.(자바는 0부터 센다)
		
		
		// 배열의 길이
		// 배열은 보통 for문과 함께 사용한다. for문에 배열이 사용될 경우 배열의 길이만큼 for 문을 돌리는 것이 중요한데
		// 배열의 길이는 다음과 같이 length를 이용하여 구한다
		for(int i = 0; i < weeks.length; i++ ) {
			System.out.println(weeks[i]);
		}
		// weeks 배열을 순서대로 출력하는 프로그램이다. length를 어떻게 사용했지 눈여겨 보도록한다
